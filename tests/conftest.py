...
def mock_fortigate_api():
    ...
    mock_api.get_dns_settings = AsyncMock(
        return_value={"results": {"primary": "8.8.8.8", "secondary": "1.1.1.1"}}
    )

    mock_api.get_dns_databases = AsyncMock(
        return_value={"results": [{"name": "example.test", "domain": "example.test"}]}
    )

    mock_api.get_dns_database_detail = AsyncMock(
        return_value={
            "results": {
                "name": "example.test",
                "domain": "example.test",
    ...
    mock_api.create_dns_database = AsyncMock(return_value={"status": "success"})
    mock_api.update_dns_database = AsyncMock(return_value={"status": "success"})
    mock_api.delete_dns_database = AsyncMock(return_value={"status": "success"})

    mock_api.get_dns_servers = AsyncMock(
        return_value={"results": [{"name": "port1", "mode": "recursive"}]}
    )
    mock_api.get_dns_server_detail = AsyncMock(
        return_value={"results": {"name": "port1", "mode": "recursive"}}
    )
    mock_api.create_dns_server = AsyncMock(return_value={"status": "success"})
    mock_api.update_dns_server = AsyncMock(return_value={"status": "success"})
    mock_api.delete_dns_server = AsyncMock(return_value={"status": "success"})

    mock_api.get_dhcp_servers = AsyncMock(
        return_value={
            "results": [
                {"id": 1, "interface": "port1", "default-gateway": "192.168.1.1"}
            ]
        }
    )

    mock_api.get_dhcp_server_detail = AsyncMock(
        return_value={
            "results": {"id": 1, "interface": "port1", "default-gateway": "192.168.1.1"}
        }
    )
    mock_api.create_dhcp_server = AsyncMock(
        return_value={"status": "success", "mkey": "1"}
    )
    ...
    mock_api.get_dhcp_leases = AsyncMock(
        return_value={
            "results": [
                {"ip": "192.168.1.10", "mac": "00:11:22:33:44:55", "status": "leased"}
            ]
        }
    )

    mock_api.get_interfaces = AsyncMock(
        return_value={
            "results": [
    ...
    mock_api.get_interface_status = AsyncMock(
        return_value={"results": {"name": "port1", "status": "up", "ip": "192.168.1.1"}}
    )

    mock_api.get_firewall_policies = AsyncMock(
        return_value={
            "results": [{"policyid": 1, "name": "Allow_HTTP", "action": "accept"}]
        }
    )

    mock_api.get_firewall_policy_detail = AsyncMock(
        return_value={
            "results": {
                "policyid": 35,
    ...
    mock_api.get_address_objects = AsyncMock(
        return_value={"results": [{"name": "test_addr", "subnet": "192.168.1.0/24"}]}
    )
    mock_api.get_address_object_detail = AsyncMock(
        return_value={"results": {"name": "test_addr", "subnet": "192.168.1.0/24"}}
    )
    ...
    mock_api.get_service_objects = AsyncMock(
        return_value={"results": [{"name": "HTTP", "tcp-portrange": "80"}]}
    )
    mock_api.get_service_object_detail = AsyncMock(
        return_value={"results": {"name": "HTTP", "tcp-portrange": "80"}}
    )
    ...
    mock_api.get_static_routes = AsyncMock(
        return_value={"results": [{"dst": "10.0.0.0/8", "gateway": "192.168.1.1"}]}
    )
    ...
    mock_api.get_static_route_detail = AsyncMock(
        return_value={
            "results": {"seq-num": 1, "dst": "10.0.0.0/8", "gateway": "192.168.1.1"}
        }
    )

    mock_api.get_routing_table = AsyncMock(
        return_value={"results": [{"dst": "0.0.0.0/0", "gateway": "192.168.1.1"}]}
    )

    mock_api.get_virtual_ips = AsyncMock(
        return_value={
            "results": [
                {"name": "test_vip", "extip": "1.2.3.4", "mappedip": "10.0.0.1"}
            ]
        }
    )
    ...
    mock_api.get_virtual_ip_detail = AsyncMock(
        return_value={
            "results": {"name": "test_vip", "extip": "1.2.3.4", "mappedip": "10.0.0.1"}
        }
    )
    mock_api.get_virtual_servers = AsyncMock(
        return_value={
            "results": [
                {
                    "name": "test_vs",
    ...
    mock_api.get_virtual_server_detail = AsyncMock(
        return_value={
            "results": {
                "name": "test_vs",
                "type": "server-load-balance",
    ...
    mock_api.get_virtual_server_status = AsyncMock(
        return_value={
            "runtime_status_available": True,
            "results": {"name": "test_vs", "status": "up"},
        }
    )
    mock_api.get_load_balance_health_checks = AsyncMock(
        return_value={
            "results": [{"name": "http-monitor", "type": "http", "interval": 5}]
        }
    )
    mock_api.get_load_balance_health_check_detail = AsyncMock(
        return_value={
            "results": {"name": "http-monitor", "type": "http", "interval": 5}
        }
    )
    mock_api.create_load_balance_health_check = AsyncMock(
        return_value={"status": "success"}
    )
    mock_api.update_load_balance_health_check = AsyncMock(
        return_value={"status": "success"}
    )
    mock_api.delete_load_balance_health_check = AsyncMock(
        return_value={"status": "success"}
    )
